...
"""W64 convergence instrumentation: deterministic rollout + critic-init RNG.
...
def maybe_seeded_value_head_init(value_head: Any, init_seed: Any) -> None:
    ...
    Both branches log ``W64_CRITIC_INIT`` (seeded vs skipped + the seed value
    ...
    if init_seed is None:
        _w64_log("W64_CRITIC_INIT skipped: value_head_init_seed=None (unseeded default)")
        value_head._init_weights()
        return
    devices = {p.device.type for p in value_head.parameters()}
    assert devices <= {"cpu"}, (
        f"W64 seeded critic init expects CPU parameters, got {sorted(devices)}; "
    ...
    with torch.random.fork_rng(devices=[]):
        ...
        value_head._init_weights()
    _w64_log(
        f"W64_CRITIC_INIT seed={int(init_seed)} devices={sorted(devices)} "
