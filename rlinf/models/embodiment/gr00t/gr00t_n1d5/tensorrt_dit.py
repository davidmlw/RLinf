...
"""Refittable TensorRT DiT executor for GR00T N1.5 rollout workers."""
...
_RECEIPT_SCHEMA = "rlinf.gr00t-n1d5-stack-cube-true-b8-refittable-dit-engine.v1"
_EXPECTED_INPUTS = {
    "sa_embs": ([8, 49, 1536], "BF16"),
    "vl_embs": ([8, 570, 2048], "BF16"),
    "timestep": ([8], "INT64"),
}
_EXPECTED_OUTPUT = ([8, 49, 1024], "BF16")
_EXPECTED_REFIT_TENSORS = 232
_MIN_PROBE_COSINE = 0.999
_MAX_PROBE_RELATIVE_L2 = 0.05
_MIN_FREE_DEVICE_BYTES = 8 << 30
_UNQUALIFIED_PPO_AUTHORITY = "unqualified_requires_convergence_validation"
...
def _load_artifacts(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(entries) != _EXPECTED_REFIT_TENSORS:
        raise RuntimeError(
            "refittable N1.5 DiT map must contain exactly "
            f"{_EXPECTED_REFIT_TENSORS} tensors"
        )
    return {
...
def _validate_bindings(engine: PersistentEngine, receipt: Mapping[str, Any]) -> None:
    ...
    if len(actual) != 4 or len(expected) != 4:
        raise RuntimeError("refittable N1.5 DiT engine must expose four bindings")
    ...
    if inputs != _EXPECTED_INPUTS or outputs != [_EXPECTED_OUTPUT]:
        raise RuntimeError(
            "refittable DiT engine does not implement the W06 N1.5 B8 ABI"
        )
...
class RefittableTensorRTDiT:
    ...
    def __init__(
        ...
        if self.expected_revision != 0:
            raise ValueError("W06 diagnostic supports only the initial revision zero")
        ...
        if self.online_refit:
            ...
            if self.ppo_authority_status != _UNQUALIFIED_PPO_AUTHORITY:
                raise ValueError(
                    "online TensorRT DiT must declare that convergence validation "
                    "is still required"
                )
    ...
    def verify_revision(self, revision: int) -> None:
        ...
        if revision != self.expected_revision and not self.online_refit:
            raise RuntimeError(
                "W06 revision-zero diagnostic refuses online updates: "
    ...
    def __call__(
        ...
        if encoder_attention_mask is not None:
            raise ValueError(
                "N1.5 TensorRT DiT does not consume encoder_attention_mask"
            )
        if return_all_hidden_states:
            raise ValueError(
                "N1.5 TensorRT DiT does not support return_all_hidden_states"
            )
...
# Compatibility alias for revision-zero W06 diagnostic configs and receipts.
