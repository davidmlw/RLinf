...
"""Persistent-buffer TensorRT engine for RLinf hybrid inference runtimes."""
...
class PersistentEngine:
    ...
    def binding_manifest(self) -> list[dict[str, Any]]:
        """Describe the deserialized engine bindings and optimization profile."""

        manifest = []
        for index, name in enumerate(self.handle):
            mode = self.handle.get_tensor_mode(name)
            shape = tuple(self.handle.get_tensor_shape(name))
            profile = None
            if mode == self._trt.TensorIOMode.INPUT and any(
                dimension < 0 for dimension in shape
            ):
                minimum, optimum, maximum = self.handle.get_tensor_profile_shape(
                    name, 0
                )
                profile = {
                    "min": list(minimum),
            ...
            manifest.append(
                {
                    ...
                    "dtype": str(self.handle.get_tensor_dtype(name)),
                    "shape": list(shape),
                    "profile": profile,
                }
            )
    ...
    def telemetry(self) -> dict[str, Any]:
        return {
            ...
            "closed": self.closed,
            "bindings": self.binding_manifest() if self.handle is not None else [],
        }
