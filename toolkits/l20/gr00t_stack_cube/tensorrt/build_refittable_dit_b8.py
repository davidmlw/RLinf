...
EXPECTED_BATCH = 8
EXPECTED_ACTION_SEQUENCE = 49
EXPECTED_VL_SEQUENCE = 570
EXPECTED_REFIT_WEIGHTS = 232
...
def _validate_bindings(bindings: list[dict[str, Any]]) -> None:
    ...
    expected = {
        ...
        "vl_embs": [EXPECTED_BATCH, EXPECTED_VL_SEQUENCE, 2048],
    ...
    if any(item["profile"] is not None for item in inputs.values()):
        raise RuntimeError("N1.5 W06 DiT bindings must be fully static true-B8")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if mapping["dit_refit"]["initializer_count"] != EXPECTED_REFIT_WEIGHTS:
        raise RuntimeError(
            "offline parameter map did not pass the "
            f"{EXPECTED_REFIT_WEIGHTS}-weight gate"
        )
    ...
    for index in range(network.num_inputs):
        ...
        if any(dimension < 0 for dimension in shape):
            raise RuntimeError(
                f"N1.5 W06 DiT input {value.name} is not fully static: {shape}"
            )
    ...
    receipt = {
        "schema": "rlinf.gr00t-n1d5-stack-cube-true-b8-refittable-dit-engine.v1",
        "status": "passed",
...
def main() -> int:
    ...
    try:
        ...
        traceback.print_exc()
        print(f"W06 N1.5 refittable DiT build failed: {error}", file=sys.stderr)
        return 1
