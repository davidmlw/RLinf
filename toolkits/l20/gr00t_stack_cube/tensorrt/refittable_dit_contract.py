...
"""Build the offline parameter contract for a refittable GR00T N1.5 DiT.
...
def validate_model_config(path: Path) -> dict[str, Any]:
    ...
    value = root.get("action_head_cfg", root)
    mismatches = {
    ...
    if mismatches:
        raise ValueError(f"model config does not match the W06 contract: {mismatches}")
    return {
...
def build_refit_manifest(
    ...
    return {
        "schema": "rlinf.gr00t-n1d5-dit-refit-parameter-map.v1",
        "status": "passed",
...
def _main() -> None:
    ...
    value = {
        "schema": "rlinf.gr00t-n1d5-refittable-dit-offline-gate.v1",
        "status": "passed",
        "inputs": {
