...
if [[ $# -ne 2 ]]; then
    echo "Usage: run_w09.sh ATTEMPT MAX_STEPS" >&2
    exit 2
fi

ATTEMPT=$1
MAX_STEPS=$2
if [[ ! "$ATTEMPT" =~ ^W09-[A-Za-z0-9][A-Za-z0-9._-]*$ ]]; then
    echo "ATTEMPT must start with W09- and contain only safe path characters" >&2
    exit 2
fi
...
REMOTE_WORKSPACE=${W09_REMOTE_WORKSPACE:-/home/liweim/rl-workspace/rlinf}
W06_ROOT=${W09_W06_ROOT:-$REMOTE_WORKSPACE/w06-n15-l20-trt-backbone}
TRT_ROOT=${W09_TRT_ROOT:-$REMOTE_WORKSPACE/runtime/W06/tensorrt-10.15.1.29}
...
for path in \
    ...
    if [[ ! -e "$path" ]]; then
        echo "required W09 input is missing: $path" >&2
        exit 1
    fi
done

export RUN_SERIES=W09
export CONFIG_NAME=isaaclab_franka_stack_cube_ppo_gr00t_w08
export W08_ENABLE_DIT=true
export W08_PPO_GATE_ENABLED=false
export W08_EVAL_TOTAL_NUM_ENVS=64
export EXTRA_PYTHONPATH=$TRT_ROOT
export EXTRA_LD_LIBRARY_PATH=$TRT_ROOT/tensorrt_libs
export W05_TMP_ROOT=${W09_TMP_ROOT:-$HOME/r/w09}
export SAVE_INTERVAL=${SAVE_INTERVAL:-5}
export VAL_CHECK_INTERVAL=${VAL_CHECK_INTERVAL:-5}
