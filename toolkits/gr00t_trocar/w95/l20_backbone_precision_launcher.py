...
"""Launch the W98 L20 BF16-ViT datatype-correction qualification."""
...
class LaunchError(RuntimeError):
    """Raised when the W98 datatype-correction launch contract fails."""
...
def _extra_docker_args(source: Path, old_artifacts: Path) -> tuple[str, ...]:
    return (
        "-e",
...
def _agent_command(revision: str) -> str:
    return (
        ...
        "/workspace/w98-src/toolkits/gr00t_trocar/w95/"
        "l20_backbone_precision_agent.py "
        "--source /workspace/w98-src --gr00t-source /workspace/gr00t-n17 "
        f"--rlinf-revision {revision} --old-artifacts /w98-old "
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    old_artifacts = args.old_artifacts.resolve(strict=True)
    ...
    agent = Path(__file__).with_name("l20_backbone_precision_agent.py")
    shutil.copy2(agent, receipts / "agent.py")
    _write(
        receipts / "inputs.json",
        {
            "schema": "rlinf.w98.l20-bf16-vit-inputs/v1",
            "source": source_identity,
            "old_artifacts": str(old_artifacts),
            "w96_static": static,
            "site_sha256": _sha256(args.site),
            "launcher_sha256": _sha256(Path(__file__)),
            "agent_sha256": _sha256(agent),
            "scope": "standalone datatype correction; no RL/PPO/E2E",
        },
    )
    preflight = {
    ...
    container = runtime._run_container(
        ...
        extra_args=_extra_docker_args(args.source.resolve(), old_artifacts),
    )
    result_path = run_root / "artifacts/result.json"
    result = runtime._load(result_path)
    if result.get("status") not in {"passed", "failed"}:
        raise LaunchError(f"datatype correction did not produce a result: {result}")
    output = {
        "schema": "rlinf.w98.l20-bf16-vit-launch/v1",
        "status": "passed" if result["status"] == "passed" else "completed_failed_gate",
        "source": source_identity,
        "container": container,
        "experiment_result": {"path": str(result_path), "sha256": _sha256(result_path)},
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"W98 L20 BF16-ViT qualification failed: {error}", file=sys.stderr)
        return 1
