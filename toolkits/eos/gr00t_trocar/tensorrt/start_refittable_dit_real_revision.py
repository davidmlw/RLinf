...
"""Submit the EOS W83 two-slot real Actor-revision refit gate."""
...
import start_refittable_dit_probe as probe_workflow

DEFAULT_CHECKPOINT = (
    build_workflow.WORKSPACE
    / "runs/W81/W81-hybrid-trt-eager-reuse-5968857/output"
    / "w81_n1d7_hybrid_trt_eager_reuse/checkpoints/global_step_5"
    / "actor/model_state_dict/full_weights.pt"
)


def _qualified_checkpoint(path: Path) -> Path:
    value = path.resolve(strict=True)
    if value.name != "full_weights.pt" or value.stat().st_size < (1 << 30):
        raise build_workflow.WorkflowError(
            "real revision input must be a retained full Actor checkpoint"
        )
    return value
...
def _submit(args: argparse.Namespace) -> int:
    ...
    qualified = probe_workflow._qualified_build(args.build_attempt)
    checkpoint = _qualified_checkpoint(args.checkpoint)
    launcher = (
        build_workflow.SOURCE
        / "toolkits/eos/gr00t_trocar/tensorrt/start_refittable_dit_real_revision.py"
    )
    allocation = [
        "python3",
        str(launcher),
    ...
    build_workflow._write_new(
        build_workflow.RUN_ROOT / f"submission-{job_id}.json",
        {
            "job_id": job_id,
            "revision": revision,
    ...
    print(json.dumps({"job_id": job_id, "revision": revision}, indent=2))
    return 0
...
def _run_agent(args: argparse.Namespace) -> int:
    ...
    command = [
        sys.executable,
        str(tools / "refit_dit_real_revision_probe.py"),
        "--source",
    ...
    result = {
        "schema": "rlinf.w83-refittable-dit-real-revision.v1",
        "status": "passed",
        "revision": args.revision,
        "build_attempt": str(qualified),
        "checkpoint": str(checkpoint),
        ...
        "probe_receipt_sha256": build_workflow._sha256(
            output / "real-revision-refit-receipt.json"
        ),
    }
...
def main() -> int:
    ...
    submit.add_argument("--attempt-name", default="W83-refittable-dit-real-r1")
    submit.add_argument("--time-limit", default="01:00:00")
    submit.add_argument("--build-attempt", type=Path, required=True)
    submit.add_argument("--checkpoint", type=Path, default=DEFAULT_CHECKPOINT)
    submit.add_argument("--checkpoint-revision", type=int, default=5)
    ...
    allocation.add_argument("--checkpoint", type=Path, required=True)
    allocation.add_argument("--checkpoint-revision", type=int, required=True)
    ...
    agent.add_argument("--checkpoint-revision", type=int, required=True)
    ...
    try:
        ...
        traceback.print_exc()
        print(f"W83 real revision workflow failed: {error}", file=sys.stderr)
        return 1
